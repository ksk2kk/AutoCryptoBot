...
use self::rest::OkxRestClient;
use self::ws::OkxWsManager;
...
impl OkxConnector {
    pub fn new(rest_url: &str, ws_url: &str) -> Self {
        Self {
            rest: OkxRestClient::new(rest_url),
            ws: OkxWsManager::new(ws_url),
...
impl ExchangeConnector for OkxConnector {
    ...
    async fn connect(&self) -> Result<()> {
        ...
        tracing::info!(exchange = "okx", "Connected to OKX");
        Ok(())
    }
    ...
    async fn disconnect(&self) -> Result<()> {
        ...
        tracing::info!(exchange = "okx", "Disconnected from OKX");
        Ok(())
    }
